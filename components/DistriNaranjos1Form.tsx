...
export default function DistriNaranjos1Form() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      const savedStep = localStorage.getItem('distriNaranjos1_step');
      const savedCart = localStorage.getItem('distriNaranjos1_cart');
      const savedClient = localStorage.getItem('distriNaranjos1_client');
      
      console.log('🔍 Loading DistriNaranjos1 localStorage data:', {
        savedStep,
  ...
  useEffect(() => {
    if (isClient && typeof window !== 'undefined') {
      localStorage.setItem('distriNaranjos1_step', JSON.stringify(currentStep));
      console.log('💾 Saving DistriNaranjos1 step:', currentStep);
    }
  }, [currentStep, isClient]);

  useEffect(() => {
    if (isClient && typeof window !== 'undefined') {
      localStorage.setItem('distriNaranjos1_cart', JSON.stringify(cartItems));
      console.log('💾 Saving DistriNaranjos1 cart:', cartItems.length, 'items');
    }
  }, [cartItems, isClient]);

  useEffect(() => {
    if (isClient && typeof window !== 'undefined') {
      localStorage.setItem('distriNaranjos1_client', JSON.stringify(client));
  ...
  const handleGeneratePDF = async (comentario: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        localStorage.removeItem('distriNaranjos1_step');
        localStorage.removeItem('distriNaranjos1_cart');
        localStorage.removeItem('distriNaranjos1_client');
          ...
          <div className="max-w-2xl mx-auto">
            ...
            <div className="bg-white rounded-lg shadow-lg p-6">
              <ClientForm 
                ...
                storageKeyPrefix="distriNaranjos1"
              />
            </div>
          </div>
        )}
              ...
              <div className="flex items-center space-x-3">
                <div className="relative">
                  <button
                    ...
                    className="relative bg-green-600 text-white px-4 py-2 rounded-full hover:bg-green-700 transition-all duration-200 font-medium flex items-center space-x-2 shadow-lg hover:shadow-xl transform hover:scale-105"
              ...
              <div className="flex items-center gap-2">
                {cartItems.length > 0 && (
                  <button
                    onClick={() => {
                      try {
                        localStorage.setItem('distriNaranjos1_saved_cart', JSON.stringify(cartItems));
                ...
                <button
                  onClick={() => {
                    try {
                      const saved = localStorage.getItem('distriNaranjos1_saved_cart');
                      if (saved) {
